...
public class Events<T extends Event> implements Listener, EventExecutor {
    ...
    public static <T extends Event> Events<T> subscribe(Class<T> clazz, EventPriority priority, boolean ignoreCancelled, Function<T, Boolean> func) {
        ...
        if (Bukkit.isPrimaryThread()) {
            Bukkit.getPluginManager().registerEvent(clazz, factory, priority, factory, BukkitPlugin.getInstance(), ignoreCancelled);
        } else {
            Bukkit.getScheduler().runTask(BukkitPlugin.getInstance(), () -> Bukkit.getPluginManager().registerEvent(clazz, factory, priority, factory, BukkitPlugin.getInstance(), ignoreCancelled));
        }
        return factory;
    }
    ...
    public void unregister() {
        ...
        try {
            ...
            if (Bukkit.isPrimaryThread()) {
                ...
                Bukkit.getScheduler().runTask(BukkitPlugin.getInstance(), () -> handlerList.unregister(this));
            }
